...
test("OpenAI Responses: 官方端点对于非原生 effort 执行安全 fallback", () => {
	...
	assert.equal(adaptedXhigh?.reasoning?.effort, "high");
	// max -> high
...
test("OpenAI Responses: 自定义第三方端点保留自定义 effort 绝不拦截", () => {
	...
	// xhigh 在三方端点原样保留
	const payloadXhigh = { model: "custom-o3", reasoning: { effort: "xhigh" } };
	const adaptedXhigh = adaptThinkingPayload(payloadXhigh, { provider: "testProvider", id: "custom-o3", api: "openai-responses" }, customState);
	assert.equal(adaptedXhigh, undefined);
	// max 在三方端点原样保留
...
test("OpenAI Completions: 官方端点安全 fallback，第三方端点原样直通", () => {
	...
	const payloadCustom = { model: "deepseek-r1", reasoning_effort: "xhigh" };
...
test("Anthropic Messages: 官方端点对 minimal 回退至 low", () => {
	...
	// 自定义端点原样保留
	const payloadCustom = { model: "claude-3-7-sonnet", output_config: { effort: "minimal" } };
